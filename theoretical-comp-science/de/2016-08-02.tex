...
\begin{document}	
	...
	\begin{solution}
		\begin{enumerate}[(i)]
			...
			\item Wahr. Gilt $A=Wb(\psi)$ für eine partiell rekursive Funktion $\psi$, so ist $f(A)=Wb(f\circ\psi)$.
			\item Falsch. Sei $f(n)$ das $n$-te Element des Halteproblems $K$ gemäß einer festen Aufzählung.
				Dann ist $f$ rekursiv, weil $K$ r.a. ist, aber $f(\mathbb{N})=K$ ist nicht rekursiv.
			\item Wahr. Ist $A=Db(\psi)$ für eine partiell rekursive Funktion $\psi$, so ist $f^{-1}(A)=Db(\psi\circ f)$.
		\end{enumerate}
	\end{solution}
